...
const BMICH = () => {
  ...
  const [numRows, setNumRows] = useState(() => {
    // Retrieve the number of rows from local storage, default to 5
    const savedRows = localStorage.getItem('numRows');
    return savedRows ? parseInt(savedRows, 10) : 5;
  });
  const [numColumns, setNumColumns] = useState(() => {
    // Retrieve the number of columns from local storage, default to 10
    const savedCols = localStorage.getItem('numColumns');
  ...
  const fetchSeats = async () => {
    try {
      ...
      const filteredSeats = response.data.filter(seat => seat.location === 'BMICH');
      setSeats(filteredSeats);
      generateSeatLayout(filteredSeats, numRows, numColumns);
    } catch (error) {
      console.error('Error fetching BMICH seats:', error);
    }
  };
  ...
  const addRow = () => {
    setNumRows(prevRows => {
      ...
      localStorage.setItem('numRows', newRowCount); // Save to local storage
  ...
  const addColumn = () => {
    setNumColumns(prevCols => {
      ...
      localStorage.setItem('numColumns', newColCount); // Save to local storage
      ...
      setNumColumns(prevCols => {
        ...
        const updatedSeats = seats
          .map(seat => {
            ...
            // If column is removed, ensure the seat ID does not exceed the new column count
          ...
          .filter(seat => parseInt(seat.seatId.slice(1), 10) <= newColCount); // Remove seats with invalid column numbers
  ...
  return (
    <div style={containerStyle}>
      <h2 style={headerStyle}>Seating Plan - BMICH</h2>
